...
class DittoModel(nn.Module):
    ...
    def __init__(self, device='cuda', lm='roberta', alpha_aug=0.8):
        ...
        if lm in lm_mp:
            ...
            if 'glm' in lm:
                self.model=AutoModel.from_pretrained(lm,trust_remote_code=True)
            else:
    ...
    def forward(self, x1, x2=None):
        ...
        if x2 is not None:
            ...
            if 'glm' in self.lm:
                for k,v in x1.items():
                    x2[k]=v.to(self.device)
            else:
                x2 = x2.to(self.device) # (batch_size, seq_len)
            
            if 'glm' in self.lm:
                for k,v in x1.items():
                    x1[k]=torch.cat((v,x2[k]))
                ...
                # print(torch.arange(len(x1), device=input_ids.device).shape)
                last_hidden_states=self.model(**x1).last_hidden_states
                enc=last_hidden_states[torch.arange(len(x1['input_ids']), device=input_ids.device), sequence_lengths]
                # enc=self.model(**x1).last_hidden_states[:, 0, :]
            else:
                enc = self.model(torch.cat((x1, x2)))[0][:, 0, :]
                
            batch_size = len(x1) if 'glm' not in self.lm else len(x2['input_ids'])
...
def train(trainset, validset, testset, run_tag, hp):
    ...
    best_dev_f1 = best_test_f1 = best_test_auc = 0.0
    ...
    for epoch in range(1, hp.n_epochs+1):
        ...
        # eval
        model.eval()
        dev_f1, dev_auc, th = evaluate(model, valid_iter)
        test_f1, test_auc,precision, recall = evaluate(model, test_iter, threshold=th)
        ...
        print(f"\nepoch {epoch}: dev_f1={dev_f1}, test_f1={test_f1},test_pre={precision},test_rec={recall}")
        print(f"best_test_pre={best_test_precision}, best_test_rec={best_test_recall},best_test_f1={best_test_f1}, best_auc_test={best_test_auc}\n")
